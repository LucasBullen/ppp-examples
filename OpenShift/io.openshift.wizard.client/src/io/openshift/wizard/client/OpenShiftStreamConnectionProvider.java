...
public class OpenShiftStreamConnectionProvider implements StreamConnectionProvider {
	...
	public boolean start() {
		...
		String serverCommand = OpenShiftProvisioningPlugin.getDefault().getPreferenceStore()
		...
		if (serverCommand.isEmpty()) {
			MessageDialog.openError(PlatformUI.getWorkbench().getActiveWorkbenchWindow().getShell(), "No Server",
					"Please specify a valid provisioning server path in the OpenShift preferences");
			return false;
		}
